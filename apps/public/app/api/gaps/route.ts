...
const throttleConfig = (data: string, env: NodeJS.ProcessEnv): VisitorThrottleConfig => ({
  ...
  ledgerFile: join(data, 'gaps-throttle.json'),
})

/** The same rolling-window machinery, keyed by the DOMAIN read rather than by who asked. */
const domainCapConfig = (data: string, env: NodeJS.ProcessEnv): VisitorThrottleConfig => ({
...
export async function GET(req: Request): Promise<Response> {
  ...
  const cycle = day ? readCycle(data, domain, day) : latestCycle(data, domain)
  if (!cycle) {
    return json({ message: day ? `this machine holds no cycle of ${domain} for ${day}` : `this machine holds no cycle of ${domain}, so there is no cycle to report the page against` }, 404)
  }
  ...
  const perDomain = domainCapConfig(data, env)
  ...
  inFlight += 1
  try {
    const report = await gapReportFor(data, domain, day ? { day } : {})
