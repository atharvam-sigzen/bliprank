import { formatFrequency, formatInterval, formatValue } from '@bliprank/stats'
import { runInfoOf, subjectOf, type ScanResultFile } from '@/lib/scan-result'
 ...
 * half of that was three competing headline numbers and no answer to the
 ...
 * could never manage.
 */
export function Headline({ scan }: { scan: ScanResultFile }) {
  const subject = subjectOf(scan)
  const metric = subject.metric
  const spoken = formatFrequency(metric)

  /*
   * THE ZERO CASE ALREADY HAS A BETTER TREATMENT ON THIS PAGE, so this yields
   * to it rather than printing the same finding twice. `formatFrequency`
   * returns a `none` shape with its upper bound and nothing here reads it: the
   * record's own paragraph says "not mentioned in any of the N answers… a real
   * result with a real upper bound, not an error", and adds the domain-label
   * caveat, which is more than a lede should carry.
   */
  if (spoken.kind === 'none') return null

  const engines = runInfoOf(scan).engines.length
  const from = `From ${metric.n} answers${engines > 0 ? ` across ${engines} ${engines === 1 ? 'engine' : 'engines'}` : ''}.`

  return (
    <div className="headline">
      <p className="headline__claim">
        AI assistants mention <strong>{subject.name}</strong> in about{' '}
