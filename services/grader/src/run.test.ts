...
describe('a live run is refused unless every gate is deliberately opened', () => {
  it('refuses without a domain', () => {
    expect(refusal(parseArgs(['--fixture'], {}))).toMatch(/no --domain/)
  })
  ...
  it('THE COST GATE: refuses without an explicit plan, and never defaults to one', () => {
    ...
    expect(refusal(parseArgs(['--domain', 'a.com'], LIVE_ENV))).toMatch(/plan not set/)
    expect(refusal(parseArgs(['--domain', 'a.com', '--plan', 'nonsense'], LIVE_ENV))).toMatch(/unknown plan/)
  })
  ...
  it('THE R3 GATE: refuses unless COLLECTION_ENABLED is exactly "true"', () => {
    ...
    for (const v of [undefined, 'false', 'TRUE', '1', 'yes', '']) {
      ...
      expect([v, refusal(parseArgs(base, env))]).toEqual([v, expect.stringMatching(/COLLECTION_ENABLED/)])
    }
  })

  it('refuses without a key', () => {
    expect(refusal(parseArgs(['--domain', 'a.com', '--plan', 'mega', '--cap', '1'], { COLLECTION_ENABLED: 'true' }))).toMatch(/API_KEY/)
  })

  it('THE CEILING GATE: a run that may spend must state its cap', () => {
    expect(refusal(parseArgs(['--domain', 'a.com', '--plan', 'mega'], LIVE_ENV))).toMatch(/no --cap/)
    for (const bad of ['0', '-1', 'abc']) {
      expect([bad, refusal(parseArgs(['--domain', 'a.com', '--plan', 'mega', '--cap', bad], LIVE_ENV))]).toEqual([bad, expect.stringMatching(/--cap must be/)])
    }
  })

  it('refuses an engine that is not in the contract', () => {
    expect(refusal(parseArgs(['--domain', 'a.com', '--fixture', '--engines', 'chatgpt,perplexity'], {}))).toMatch(/unknown engine/)
  })

  it('accepts a fully specified live run', () => {
    const r = parseArgs(['--domain', 'pipedrive.com', '--plan', 'mega', '--cap', '0.50', '--day', '2026-08-24'], LIVE_ENV)
...
describe('an offline run needs none of it, because it cannot spend', () => {
  it('fixture and stub modes skip the plan, key, cap and enablement gates', () => {
    for (const mode of ['--fixture', '--stub']) {
      const r = parseArgs(['--domain', 'pipedrive.com', mode], {})
