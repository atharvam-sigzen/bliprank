...
describe('⚠️ EVERY ATTEMPT IS CHARGED TO A DOLLAR LEDGER BEFORE IT IS MADE (R3)', () => {
  ...
  it('counts an attempt per model tried, at the configured price, in the file', async () => {
    const config = { ...CONFIG, ledger: ledger(5, 0.01) }
    // Primary fails, fallback answers: two attempts, two charges.
    let n = 0
    const bank = await authorBank(input(async () => ((n += 1) === 1 ? new Response('down', { status: 503 }) : chat(bankJson())), config))
    expect(bank?.model).toBe('fallback/model:free')
    const stored = JSON.parse(readFileSync(config.ledger.file, 'utf8')) as { calls: number; spentUsd: number; byEngine: Record<string, { calls: number }> }
    expect(stored.calls).toBe(2)
    expect(stored.spentUsd).toBeCloseTo(0.02, 9)
    expect(Object.keys(stored.byEngine).sort()).toEqual(['fallback/model:free', 'primary/model:free'])
  })

  it('⚠️ a cap that is reached means NO request, a null bank and the general bucket', async () => {
    // Room for one attempt at this price; the second is refused before fetch.
    const config = { ...CONFIG, ledger: ledger(0.015, 0.01) }
    ...
    expect(logs.some((l) => /budget exhausted/.test(l))).toBe(true)
    expect(existsSync(config.ledger.file)).toBe(true)
  })
})
